...
class GameService:
    ...
    def get_scene_characters(location_id: str, player_id: str):
        ...
        for npc_doc in query.stream():
            ...
            present_npcs.append({
                ...
                "affinity": affinity, "status": status
            })
    ...
    @staticmethod
    def process_player_action(player_id: str, action): # 暫時移除類型提示以符合延遲導入
        # (新) 將導入語句移動到函式內部
        from app.models.action import PlayerAction
        # 進行類型驗證
        if not isinstance(action, PlayerAction):
             action = PlayerAction.model_validate(action)
