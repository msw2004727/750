...
@app.route('/')
def index():
    return "文字江湖遊戲後端 v2.4 已啟動！(包含實體查詢API)"
...
def register():
    ...
    try:
        ...
        initial_world_state = {
            ...
            "pc_data": {
                "basic_info": {"name": nickname, "height": data.get('height'), "weight": data.get('weight'), "gender": data.get('gender'), "personality_trait": personality},
...
def generate_turn():
    ...
    try:
        ...
        payload = {"model": "deepseek-chat", "messages": [{"role": "system", "content": "你是一位頂尖的武俠小說家兼遊戲世界主持人(GM)，你需要根據規則生成劇情、數據指令和實體標籤。"}, {"role": "user", "content": prompt_text}], "max_tokens": 1000, "temperature": 0.8}
