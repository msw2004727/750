...
#### 角色基礎屬性（依職業）
| 職業 | HP | ATK | DEF | SPD | Range | AtkSpeed |
|------|-----|-----|-----|-----|-------|----------|
| 村民 | 30 | 3 | 1 | 3 | 1 | 1.0s |
| 步兵 | 80 | 10 | 8 | 2 | 1 | 1.2s |
| 射手 | 50 | 12 | 3 | 2 | 4 | 1.5s |
...
- 固定值在 CLASS_STATS（characterLib.js）
- 實例值在 block.state：faction, curHp, action

### 移動/時間系統
- 角色 AI 自主移動，4x4 子格系統（每格 16 站位）
- 移動 sub-tick 200ms，過一格約 800ms
- 同類角色群聚吸引（flockScore），探索未知格優先（visitScore）
- Tick 間隔：2 秒
